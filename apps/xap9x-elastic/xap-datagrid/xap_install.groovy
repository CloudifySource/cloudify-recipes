...
if(config.license!=null && config.license.size()>0){
    ...
    new AntBuilder().sequential {
        copy(file:"${context.serviceDirectory}/overwrite/gslicense.xml", tofile:"${context.serviceDirectory}/${config.installDir}/${config.xapDir}/gslicense.xml", overwrite:true)
    }
}
