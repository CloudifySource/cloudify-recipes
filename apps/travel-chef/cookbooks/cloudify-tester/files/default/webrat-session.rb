...
For example:
  ...
  class Session
    ...
    def request_page(url, http_method, data) #:nodoc:
      ...
      @current_url  = url
      @session_host = URI.parse(url).host if URI.parse(url).host
  ...
  private
    ...
    def current_host
      URI.parse(current_url).host || @custom_headers["Host"] || @session_host
    end

    def response_location_host
      URI.parse(response_location).host || @session_host
    end
