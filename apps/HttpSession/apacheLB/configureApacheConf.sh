...
# $7 sessionCookieName: i.e. "JSESSIONID"
	...
	if [ -d "$i" ] ; then
		...
		if [ -f "${currConf}" ] ; then
			...
			if [ "${useStickysession}" == "true" ] ; then
				echo "Replacing STICKYSESSION_PLACE_HOLDER with ${sessionCookieName} in ${proxyBalancerPath}"
				sudo sed -i -e "s/STICKYSESSION_PLACE_HOLDER/stickysession=${sessionCookieName} nofailover=Off/g" ${proxyBalancerPath} || error_exit $? "Failed on: useStickysession sudo sed -i -e STICKYSESSION_PLACE_HOLDER ${proxyBalancerPath}"
				sudo sed -i -e 's@STICKYSESSION_HEADER_PLACE_HOLDER@Header add Set-Cookie "'${sessionCookieName}'=.%{BALANCER_WORKER_ROUTE}e; path=/" env=BALANCER_ROUTE_CHANGED@g' ${proxyBalancerPath}|| error_exit $? "Failed on: useStickysession sudo sed -i -e STICKYSESSION_HEADER_PLACE_HOLDER with add-header"
			else
				...
				sudo sed -i -e "s/STICKYSESSION_HEADER_PLACE_HOLDER//g" ${proxyBalancerPath} || error_exit $? "Failed on: without useStickysession sudo sed -i -e STICKYSESSION_HEADER_PLACE_HOLDER ${proxyBalancerPath}"
			fi
