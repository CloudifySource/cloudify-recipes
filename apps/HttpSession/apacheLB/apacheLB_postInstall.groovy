...
if ( isLinux ) {
	println("About to configure ApacheLB. session cookie name is " + config.sessionCookieName)
	builder.sequential {		
		...
		exec(executable: "${confScript}",failonerror: "true") {
			...
			arg(value:"${config.sessionCookieName}")
		}	
	}
}
...
if( !isLinux ) {
	...
	if ( "${config.useStickysession}"=="true" ) {
		...
		proxyConfigText = proxyConfigText.replace("STICKYSESSION_PLACE_HOLDER","stickysession=${config.sessionCookieName} nofailover=Off")
	}
	else {	
