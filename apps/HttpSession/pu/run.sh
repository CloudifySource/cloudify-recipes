...
sed -i "1i export GSA_JAVA_OPTIONS=\"-Xmx512m -Xms512m -Dcom.gs.agent.auto-shutdown-enabled=true -Dcom.gs.multicast.enabled=false -Dcom.gigaspaces.lrmi.watchdog.level=FINE \"" $GSHOME/bin/gs-agent.sh || error_exit $? "Failed updating gs-agent.sh"
...
sed -i "1i export GSM_JAVA_OPTIONS=\"-Xmx512m -Xms512m -Dcom.gs.grid.gsm.pending-dispatch-delay=5000 -Dcom.sun.jini.reggie.initialUnicastDiscoveryPort=4242 -Dgsm.excludeGscOnFailedInstance.disabled=true -Dcom.gs.multicast.enabled=false\"" $GSHOME/bin/gsm.sh || error_exit $? "Failed updating gsm.sh"
...
sed -i "1i export EXT_JAVA_OPTIONS=\"-Xmx1024m -Xms512m -Dcom.sun.jini.reggie.initialUnicastDiscoveryPort=4242 -Dcom.gs.multicast.discoveryPort=4242 -Dcom.gs.multicast.enabled=false  -Dcom.gs.jini_lus.groups=$GROUP -Dcom.gs.jini_lus.locators=$LOCATORS\"" $GSHOME/bin/setenv.sh || error_exit $? "Failed updating setenv.sh"
