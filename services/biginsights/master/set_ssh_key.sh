...
-----END RSA PRIVATE KEY-----" > ~root/.ssh/id_rsa
	...
	sudo echo "$public_ip master.$DOMAIN master" >> /etc/hosts
	sudo echo "$private_ip master-internal.$DOMAIN master-internal" >> /etc/hosts
	...
	do
		...
		sudo echo "$data_node_public_ip data-1.$2 data-1" >> /etc/hosts
		sudo echo "$data_node_private_ip data-1-internal.$2 data-1-internal" >> /etc/hosts
	done
	cat $1 | while read line
	do
		[ -z "$line" ] && continue
